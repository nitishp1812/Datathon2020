...
# code for running kmeans on the best cluster

data = pd.read_csv("first_cluster.csv")
zip_map = {}
...
# generate CSV for optimal cluster
new_data = []
for i in range(len(data)):
...
new_df.to_csv('final_zipcodes.csv', index=False)

s, zips = zip(*sorted(zip(s, zips)))
